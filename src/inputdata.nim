import std/[options, strutils, streams, unicode]
...
proc parseItems(s: string): Items =
  s.splitLines() --> filter(it != "").map(block:
    let parts = it.split(';')
    let path = parts[1].strip(runes = WhitespacePlusQuoteUC).replace('\\', '/')
    (id: parts[0].strip(runes = WhitespacePlusQuoteUC),
     imagePath: path).Item)
...
proc constructObject*(s: var YamlStream, c: ConstructionContext, result: var Items)
  ...
  constructScalarItem(s, item, Items):
    result = parseItems(item.scalarContent)

proc readYamlConfig*(filepath: string): Catalogue  {.raises: [YamlConstructionError, YamlStreamError, IOError, OSError, YamlParserError, Exception].} =
...
proc readTomlConfig*(filepath: string): Catalogue {.raises: [IOError, OSError, Exception].} =
  ...
  for page in toml["pages"].getElems():
    ...
    for group in page["groups"].getElems():
      ...
      newgroup.items = group["items"].getStr("").strip(runes = WhitespacePlusQuoteUC).parseItems()
...
when isMainModule:
  ...
  suite "Test `parseItem`":
    test "two items":
      ...
      let expected = @[(id: "1", imagePath: "image1.png"),
                      (id: "2", imagePath: "image2.png")]
